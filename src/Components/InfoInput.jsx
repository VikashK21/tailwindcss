...
export default function InfoInput({ setNames }) {
  ...
  return (
    <div className="text-center">
      ...
      {isDisabled() ? (
        <div className="dark:md:hover:bg-gray-500 bg-gray-600 rounded-lg inline-block pl-2 ml-2 pr-2">
          <button disabled={true} onClick={onClick}>
            shoot
          </button>
        </div>
      ) : (
        <div className="dark:md:hover:bg-lime-700 bg-lime-600 rounded-lg inline-block pl-2 ml-2 pr-2">
          <button disabled={false} onClick={onClick}>shoot</button>
        </div>
      )}
      {/* </div> */}
    </div>
